...
class CourseManager:
    ...
    def create_or_update_course(
        ...
        if is_new_course:
            ...
            logger.info(
                f"[{self.user_email}] Creating new course: ID={course_id}, Title='{course_title}'"
            )
        else:
            logger.info(
                f"[{self.user_email}] Updating course: ID={course_id}, Title='{course_title}'"
            )
        ...
        if not is_new_course:
            try:
                ...
                raise ValueError(
                    f"Failed to retrieve existing course info for update: {course_id}"
                ) from e
        else:
            course_info = {
                "id": course_id,
    ...
    def add_uploaded_file(self, course_id: str, filename: str, filesize: int) -> bool:
        ...
        """
        logger.info(
            f"[{self.user_email}] Adding file '{filename}' ({filesize} bytes) to course {course_id}"
        )
        course_info_key = self._get_course_info_key(course_id)
        ...
        try:
            ...
            if not file_exists:
                ...
                course_info["uploadedFiles"].append({"name": filename, "size": filesize})
                course_info["last_updated_at"] = self._get_current_utc_iso_string()
                ...
                logger.warning(
                    f"File '{filename}' already exists in metadata for course {course_id}. Skipping add."
                )
    ...
    def remove_uploaded_file(self, course_id: str, filename: str) -> bool:
        ...
        try:
            ...
            if len(course_info["uploadedFiles"]) < initial_length:
                ...
                logger.warning(
                    f"File '{filename}' not found in metadata for course {course_id}. No update needed."
                )
                return True  # Return True as state is consistent (file is not listed)

        except Exception as e:
            logger.error(
                f"Error removing file metadata for course {course_id}, file {filename}: {e}"
            )
            return False
        ...
        try:
            ...
            for course_id in course_ids:
                ...
                if info:
                    ...
                    is_complete = info.get("create_course_process", {}).get(
                    ...
                    logger.warning(
                        f"Could not retrieve course info for listed course ID: {course_id}"
                    )
    ...
    def customize_course(
        self, course_id: str, title: str, progress: int, ai_tutor: dict, uploaded_files: list
    ) -> bool:
    ...
    def generate_syllabus(self, course_id: str) -> Optional[Dict]:
        ...
        try:
            ...
            try:
                ...
                s3_utils.upload_json_to_s3(self.s3_bucket, syllabus_key, mock_data)
    ...
    def get_slides(self, course_id: str) -> Optional[List[Dict[str, Any]]]:
        ...
        try:
            ...
            if slides_data and isinstance(
                slides_data, list
            ):  # Assuming slides.json directly contains the list
                logger.info(f"Successfully fetched slides from {slides_key}")
                return slides_data
            elif slides_data:  # If it's a dict with a key, adjust as needed
                logger.warning(
                    f"Slides data in {slides_key} is not a direct list. Check structure."
                )
