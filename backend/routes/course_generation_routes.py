    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            pdf_files = s3_utils.list_files_in_prefix(
                bucket="anantra-lms-store", prefix=s3_pdf_prefix, file_extension=".pdf"
            )
            ...
            if not s3_utils.download_file_from_s3(
                bucket="anantra-lms-store", s3_key=first_pdf_key, local_path=local_pdf_path
            ):
            ...
            s3_utils.upload_json_to_s3(
                json_data=result, bucket_name="anantra-lms-store", s3_key=s3_outline_path
            )
            ...
            jsonify(
                {
                    ...
                    "course_id": course_id,
                    "s3_paths": {"pdf_source": first_pdf_key, "outline": s3_outline_path},
...
def generate_slides():
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Download outline
            if not s3_utils.download_file_from_s3(
                bucket="anantra-lms-store", s3_key=s3_outline_path, local_path=local_outline_path
            ):
            ...
            for root, dirs, files in os.walk(output_dir):
                for file in files:
                    if file.endswith(".json") or file.endswith(".png"):
                        ...
                        # Upload the file
                        s3_utils.upload_file_to_s3(
                            local_path=local_file_path, bucket="anantra-lms-store", s3_key=s3_key
                        )

        return (
            jsonify({"message": "Slides generated successfully", "s3_path": slides_s3_prefix}),
            200,
        )
...
def get_course_outline():
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if not s3_utils.download_file_from_s3(
                bucket="anantra-lms-store", s3_key=s3_outline_path, local_path=local_path
            ):
...
def get_course_slides():
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if not s3_utils.download_file_from_s3(
                bucket="anantra-lms-store", s3_key=s3_slides_path, local_path=local_path
            ):
            ...
            jsonify(
                {
                    ...
                    "slides": slides_data,
                    "metadata": {"s3_path": s3_slides_path},
                }
            ),
            200,
        )
