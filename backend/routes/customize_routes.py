...
def customize_course():
    ...
    try:
        ...
        s3_utils.upload_json_to_s3(course_config, "anantra-lms-store", config_file_path)
        ...
        faiss_utils.process_course_context_s3(
            "anantra-lms-store", username, title, os.getenv("OPENAI_API_KEY")
        )
        print(f"Course context processed for {title} in S3.")

        return (
            jsonify({"message": "Course customized successfully and system prompt updated."}),
            200,
        )
...
def generate_course_materials():
    try:
        ...
        faiss_utils.process_course_context_s3(
            "anantra-lms-store", username, course_id, os.getenv("api_key")
        )
...
Please ensure that the generated syllabus reflects the true chapter divisions determined by parsing the entire text, thereby capturing the authentic structure of the book."""
        ...
        return (
            jsonify(
                {
                    "success": True,
                    "materials": {"syllabus": syllabus_response, "slides": slides_response},
                }
            ),
            200,
        )
...
def generate_course_slides():
    try:
        ...
        syllabus_data = s3_utils.get_json_from_s3(
            "anantra-lms-store", s3_utils.get_s3_file_path(username, course_id, "syllabus.json")
        )
...
def get_slides():
    try:
        ...
        slides_data = s3_utils.get_json_from_s3(
            "anantra-lms-store", s3_utils.get_s3_file_path(username, course_id, "slides.json")
        )
        # print(slides_data)
...
def get_course_structure():
    try:
        ...
        course_structure = s3_utils.get_json_from_s3(
            "anantra-lms-store",
            s3_utils.get_s3_file_path(username, course_id, "course_content.json"),
        )
