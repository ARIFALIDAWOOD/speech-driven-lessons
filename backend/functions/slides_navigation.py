...
def get_slides(course_id, username):
    ...
    slides_data = s3_utils.get_json_from_s3(
        s3_utils.SUPABASE_BUCKET_NAME, s3_utils.get_s3_file_path(username, course_id, "slides.json")
    )
    return slides_data
...
def go_to_next_slide(assistant_id, course_id, username):
    ...
    if new_position < len(slides):
        ...
        return (
            "Here is the transcript you would like to read for the next slide: "
            + slides[new_position]["transcript"]
        )
    else:
...
def go_to_previous_slide(assistant_id, course_id, username):
    ...
    if current_position > 0:
        ...
        emit_slide_change(assistant_id, new_position)
        return (
            "Here is the transcript you would like to read for the previous slide: "
            + slides[new_position]["transcript"]
        )
    else:
...
def go_to_specified_slide(assistant_id, course_id, username, slide_number):
    ...
    return (
        "Here is the transcript you would like to read for the specified slide: "
        + slides[slide_number]["transcript"]
    )
...
def go_to_viewing_slide(assistant_id, course_id, username):
    ...
    return (
        "Here is the transcript for the slide you're currently viewing: "
        + slides[viewing_position]["transcript"]
    )
