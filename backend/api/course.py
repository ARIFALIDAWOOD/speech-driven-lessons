...
def create_course():
    ...
    try:
        ...
        create_process_data = data.get(
            "create_course_process", {}
        )  # Get the nested dict or empty dict
        creation_step = create_process_data.get("current_step", 1)  # Default step 1
        is_creation_complete = create_process_data.get(
        ...
        return (
            jsonify({"error": "An unexpected error occurred during course creation/update."}),
            500,
        )
...
def customize_course():
    ...
    try:
        ...
        logger.error(
            f"Error in /customize for user {user_email}, course {data.get('id')}: {str(e)}"
        )
...
def generate_course_syllabus():
    ...
    try:
        ...
        logger.error(
            f"Error in /generate-course-syllabus for user {user_email}, course {course_id}: {str(e)}"
        )
        return jsonify({"error": str(e)}), 500
...
def fetch_course_info(course_id):
    ...
    try:
        ...
        return (
            jsonify({"message": "Course info retrieved successfully", "course": course_info}),
            200,
        )
...
def upload_file():
    ...
    try:
        ...
        # 1. Upload file to S3
        s3_upload_success = s3_utils.upload_file_to_s3(file, s3_utils.S3_BUCKET_NAME, s3_key)
        ...
        if not metadata_update_success:
            ...
            logger.critical(
                f"CRITICAL: S3 upload succeeded for {s3_key} but failed to update course_info.json for course {course_id}"
            )
            return (
                jsonify(
                    {
                        "error": "File uploaded but failed to update course metadata. Please contact support."
                    }
                ),
                500,
            )
        ...
        return (
            jsonify(
                {"message": "File uploaded and metadata updated successfully", "filename": filename}
            ),
            200,
        )
        ...
        logger.error(
            f"Error during upload process for file {filename}, course {course_id}, user {user_email}: {str(e)}"
        )
...
def delete_file():
    ...
    try:
        ...
        if not s3_delete_success:
            logger.warning(
                f"S3 delete command failed or file not found for {s3_key}, user {user_email}. Proceeding to metadata update."
            )
        ...
        if not metadata_update_success:
            ...
            logger.error(
                f"Failed to update course_info.json after attempting to delete file {filename} for course {course_id}"
            )
        ...
        logger.error(
            f"Error during delete process for file {filename}, course {course_id}, user {user_email}: {str(e)}"
        )
...
def process_course_content_endpoint(course_id):
    ...
    try:
        ...
        logger.error(
            f"Exception processing content for course {course_id}, user {user_email}: {str(e)}"
        )
...
def auto_save_content():
    ...
    try:
        ...
        if not course_id:
            ...
            return (
                jsonify({"message": "New course created successfully", "course": result_info}),
                200,
            )
        ...
        is_creation_complete = current_course.get("create_course_process", {}).get(
            "is_creation_complete", False
        )
        ...
        return (
            jsonify({"message": "Course content auto-saved successfully", "course": result_info}),
            200,
        )
    except ValueError as ve:
...
def update_course_step():
    ...
    try:
        ...
        return jsonify({"message": "Course step updated successfully", "course": result_info}), 200
    except ValueError as ve:
